...
void set_time(I2C_HandleTypeDef* hi2c, uint8_t sec, uint8_t min, uint8_t hour, uint8_t dow, uint8_t dom, uint8_t month, uint8_t year)
{
	...
	HAL_I2C_Mem_Write(hi2c, DS3231_ADDRESS, 0x00, 1, set_time, 7, 1000);
}
...
void get_time(I2C_HandleTypeDef* hi2c)
{
	...
	HAL_I2C_Mem_Read(hi2c, DS3231_ADDRESS, 0x00, 1, get_time, 7, 1000);
...
float get_temp(I2C_HandleTypeDef* hi2c)
{
	...
	HAL_I2C_Mem_Read(hi2c, DS3231_ADDRESS, 0x11, 1, temp, 2, 1000);
...
void force_temp_conv(I2C_HandleTypeDef* hi2c)
{
	...
	HAL_I2C_Mem_Read(hi2c, DS3231_ADDRESS, 0x0F, 1, &status, 1, 100);  // read status register
	if (!(status&0x04))
	{
		HAL_I2C_Mem_Read(hi2c, DS3231_ADDRESS, 0x0E, 1, &control, 1, 100);  // read control register
		HAL_I2C_Mem_Write(hi2c, DS3231_ADDRESS, 0x0E, 1, (uint8_t *)(control|(0x20)), 1, 100);
	}
}
