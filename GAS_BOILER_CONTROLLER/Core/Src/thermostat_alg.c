...
void thermostat_task(void)
{
	...
	if ((ram_ptr->temperature + ram_ptr->mirrored_to_rom_regs.unig.gas_boiler.temp_range) 
		<= ram_ptr->mirrored_to_rom_regs.unig.gas_boiler.temp_setpoint)
	{
		ram_ptr->rele_status = 1;
		enable_rele;
	}
	// если температура больше уставки, то выключаем реле
	else if (ram_ptr->temperature > ram_ptr->mirrored_to_rom_regs.unig.gas_boiler.temp_setpoint)
	{
		ram_ptr->rele_status = 0;
		disable_rele;
	}
}