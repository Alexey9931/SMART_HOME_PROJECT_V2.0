...
void receive_packet(w5500_data* w5500_n, uint8_t sn)
{
	...
	if(get_socket_status(w5500_n, sn) == SOCK_ESTABLISHED)
	{
		...
		memcpy(&rx_packet.end_fields, w5500_n->rx_buf + sizeof(packet_header) + rx_packet.header_fields.length - sizeof(rx_packet.end_fields.crc)
			- sizeof(rx_packet.header_fields) + sizeof(rx_packet.header_fields.header), sizeof(rx_packet.end_fields));
