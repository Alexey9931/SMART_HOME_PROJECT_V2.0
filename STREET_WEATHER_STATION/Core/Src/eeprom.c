...
void eeproms_first_ini()
{
	...
	memcpy(rom_struct.ip_addr_1, ip_addr_ini, sizeof(ip_addr_ini));
	...
	memcpy(rom_struct.mac_addr_1, mac_addr_ini, sizeof(mac_addr_ini));
	
	eeprom_page_erase(1, 0);
	eeprom_page_erase(1, 1);
...
void _eeprom_write(uint8_t rom_num, uint16_t page, uint16_t offset, uint8_t *data, uint16_t size)
{
	...
	for (int i = 0; i < numofpages; i++)
	{
		...
		HAL_I2C_Mem_Write(&hi2c1, eeprom_addr, MemAddress, 2, &data[pos], bytesremaining, 10);
...
void _eeprom_read(uint8_t rom_num, uint16_t page, uint16_t offset, uint8_t *data, uint16_t size)
{
	...
	for (int i = 0; i < numofpages; i++)
	{
		uint16_t MemAddress = startPage<<paddrposition | offset;
		uint16_t bytesremaining = bytestowrite(size, offset);
		HAL_I2C_Mem_Read(&hi2c1, eeprom_addr, MemAddress, 2, &data[pos], bytesremaining, 10);
		startPage++;
		offset = 0;
		size = size-bytesremaining;
		pos += bytesremaining;
...
void eeprom_page_erase(uint8_t rom_num, uint16_t page)
{
	...
	HAL_I2C_Mem_Write(&hi2c1, eeprom_addr, MemAddress, 2, data, PAGE_SIZE, 10);
