...
void get_time(void)
{
	...
	time.seconds = binary_to_dec(get_time[0]);
	time.minutes = binary_to_dec(get_time[1]);
	time.hour = binary_to_dec(get_time[2]);
	time.dayofweek = binary_to_dec(get_time[3]);
	time.dayofmonth = binary_to_dec(get_time[4]);
	time.month = binary_to_dec(get_time[5]);
	time.year = binary_to_dec(get_time[6]);
}
